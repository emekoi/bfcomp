...
__attribute__((malloc)) void *_dmt_alloc(size_t, int, const char *, unsigned);
__attribute__((malloc)) void *_dmt_realloc(void *, size_t, const char *,
                                           unsigned);
...
void *_dmt_realloc(void *ptr, size_t sz, const char *file, unsigned line) {
  ...
  if (ptr == NULL)
    return _dmt_alloc(sz, 0, file, line);
...
void dmt_dump(FILE *fp) {
  ...
  while (node != NULL) {
    fprintf(fp, "Unfreed: %p %s, line %lu (%lu bytes)\n",
            (char *)node + sizeof(*node), node->file, (unsigned long)node->line,
