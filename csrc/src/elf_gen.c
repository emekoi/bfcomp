 ...
 *   - we have to read large amounts of the file to get data that we want though.
 * */
...
program_t *gen_program_header(elf_gen_ctx *ctx, const char *name, Elf32_Phdr header) {
  header.p_align = PAGE_SIZE;
  map_set(&ctx->segments, name, ((program_t){.header = header}));
  return map_get(&ctx->segments, name);
}

section_t *gen_section_header(elf_gen_ctx *ctx, const char *name, Elf32_Shdr header) {
...
void fix_header_offsets(elf_gen_ctx *ctx) {
  ...
  map_iter_t iter = map_iter(&ctx->segments);
  uint64_t offset = align_to(sizeof(ctx->elf_header) + ctx->segments.count * sizeof(Elf32_Phdr), PAGE_SIZE);
  while ((name = map_next(&ctx->segments, &iter))) {
    program_t *program = map_get(&ctx->segments, name);
  ...
  ctx->sections.shstrtab = (section_t){
    .header = (Elf32_Shdr){
      .sh_name = 1,
      .sh_type = SHT_STRTAB,
...
void finalize_elf_ctx(elf_gen_ctx *ctx) {
  ...
  ctx->elf_header.e_shoff = sizeof(Elf32_Ehdr) + ctx->segments.count * sizeof(Elf32_Phdr);
}
